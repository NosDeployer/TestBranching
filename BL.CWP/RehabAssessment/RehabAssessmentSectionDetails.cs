...
{
    /// <summary>
    /// RehabAssessmentSectionDetails
    /// </summary>
    public class RehabAssessmentSectionDetails : TableModule
    {
        // ////////////////////////////////////////////////////////////////////////
        // CONSTRUCTORS
        //
        ...
        /// Constructor
        /// </summary>
        public RehabAssessmentSectionDetails()
            : base("SectionDetails")
        {
        }
        ...
        public RehabAssessmentSectionDetails(DataSet data)
        ...
        {
            RehabAssessmentSectionDetailsGateway rehabAssessmentSectionDetailsGateway = new RehabAssessmentSectionDetailsGateway(Data);
            rehabAssessmentSectionDetailsGateway.LoadByWorkId(workId, companyId);
        ...
        {
            RehabAssessmentTDS.SectionDetailsRow row = GetRow(workId);
            ...
            if (thickness.Trim() != "") row.Thickness = thickness; else row.SetThicknessNull();
        }
        ...
        public void Save(Int64? countryId, Int64? provinceId, Int64? countyId, Int64? cityId, int projectId, int companyId)
        {
            ...
            if (RehabAssessmentChanges.SectionDetails.Rows.Count > 0)
            {
                RehabAssessmentSectionDetailsGateway rehabAssessmentSectionDetailsGateway = new RehabAssessmentSectionDetailsGateway(RehabAssessmentChanges);

                // Update sections
                foreach (RehabAssessmentTDS.SectionDetailsRow sectionDetailsRow in (RehabAssessmentTDS.SectionDetailsDataTable)RehabAssessmentChanges.SectionDetails)
                {
                    // Unchanged values
                    ...
                    // Original values
                    string originalStreet = rehabAssessmentSectionDetailsGateway.GetStreetOriginal(workId);
                    int? originalUsmh = rehabAssessmentSectionDetailsGateway.GetUsmh(workId);
                    string originalUsmhId = rehabAssessmentSectionDetailsGateway.GetUsmhDescriptionOriginal(workId);
                    int? originalDsmh = rehabAssessmentSectionDetailsGateway.GetDsmh(workId);
                    string originalDsmhId = rehabAssessmentSectionDetailsGateway.GetDsmhDescriptionOriginal(workId);
                    string originalMapSize = rehabAssessmentSectionDetailsGateway.GetMapSizeOriginal(workId);
                    string originalSize = rehabAssessmentSectionDetailsGateway.GetSizeOriginal(workId);
                    string originalMapLength = rehabAssessmentSectionDetailsGateway.GetMapLengthOriginal(workId);
                    string originalLength = rehabAssessmentSectionDetailsGateway.GetLengthOriginal(workId);
                    int? originalLaterals = rehabAssessmentSectionDetailsGateway.GetLateralsOriginal(workId);
                    int? originalLiveLaterals = rehabAssessmentSectionDetailsGateway.GetLiveLateralsOriginal(workId);
                    string originalUsmhDepth = rehabAssessmentSectionDetailsGateway.GetUsmhDepthOriginal(workId);
                    string originalDsmhDepth = rehabAssessmentSectionDetailsGateway.GetDsmhDepthOriginal(workId);
                    string originalSteelTapeThroughSewer = rehabAssessmentSectionDetailsGateway.GetSteelTapeThroughSewerOriginal(workId);
                    string originalUsmhMouth12 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth12Original(workId);
                    string originalUsmhMouth1 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth1Original(workId);
                    string originalUsmhMouth2 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth2Original(workId);
                    string originalUsmhMouth3 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth3Original(workId);
                    string originalUsmhMouth4 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth4Original(workId);
                    string originalUsmhMouth5 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth5Original(workId);
                    string originalDsmhMouth12 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth12Original(workId);
                    string originalDsmhMouth1 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth1Original(workId);
                    string originalDsmhMouth2 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth2Original(workId);
                    string originalDsmhMouth3 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth3Original(workId);
                    string originalDsmhMouth4 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth4Original(workId);
                    string originalDsmhMouth5 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth5Original(workId);
                    string originalUsmhAddress = rehabAssessmentSectionDetailsGateway.GetUsmhAddressOriginal(workId);
                    string originalDsmhAddress = rehabAssessmentSectionDetailsGateway.GetDsmhAddressOriginal(workId);
                    string originalSubArea = rehabAssessmentSectionDetailsGateway.GetSubAreaOriginal(workId);
                    string originalThickness = rehabAssessmentSectionDetailsGateway.GetThicknessOriginal(workId);

                    // New variables
                    string newStreet = rehabAssessmentSectionDetailsGateway.GetStreet(workId);
                    string newUsmh = rehabAssessmentSectionDetailsGateway.GetUsmhDescription(workId);
                    string newDsmh = rehabAssessmentSectionDetailsGateway.GetDsmhDescription(workId);
                    string newMapSize = rehabAssessmentSectionDetailsGateway.GetMapSize(workId);
                    string newSize = rehabAssessmentSectionDetailsGateway.GetSize_(workId);
                    string newMapLength = rehabAssessmentSectionDetailsGateway.GetMapLength(workId);
                    string newLength = rehabAssessmentSectionDetailsGateway.GetLength(workId);
                    int? newLaterals = rehabAssessmentSectionDetailsGateway.GetLaterals(workId);
                    int? newLiveLaterals = rehabAssessmentSectionDetailsGateway.GetLiveLaterals(workId);
                    string newUsmhDepth = rehabAssessmentSectionDetailsGateway.GetUsmhDepth(workId);
                    string newDsmhDepth = rehabAssessmentSectionDetailsGateway.GetDsmhDepth(workId);
                    string newSteelTapeThroughSewer = rehabAssessmentSectionDetailsGateway.GetSteelTapeThroughSewer(workId);
                    string newUsmhMouth12 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth12(workId);
                    string newUsmhMouth1 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth1(workId);
                    string newUsmhMouth2 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth2(workId);
                    string newUsmhMouth3 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth3(workId);
                    string newUsmhMouth4 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth4(workId);
                    string newUsmhMouth5 = rehabAssessmentSectionDetailsGateway.GetUsmhMouth5(workId);
                    string newDsmhMouth12 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth12(workId);
                    string newDsmhMouth1 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth1(workId);
                    string newDsmhMouth2 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth2(workId);
                    string newDsmhMouth3 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth3(workId);
                    string newDsmhMouth4 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth4(workId);
                    string newDsmhMouth5 = rehabAssessmentSectionDetailsGateway.GetDsmhMouth5(workId);
                    string newUsmhAddress = rehabAssessmentSectionDetailsGateway.GetUsmhAddress(workId);
                    string newDsmhAddress = rehabAssessmentSectionDetailsGateway.GetDsmhAddress(workId);
                    string newSubArea = rehabAssessmentSectionDetailsGateway.GetSubArea(workId);
                    string newThickness = rehabAssessmentSectionDetailsGateway.GetThickness(workId);
        ...
        private RehabAssessmentTDS.SectionDetailsRow GetRow(int workId)
        {
            RehabAssessmentTDS.SectionDetailsRow row = ((RehabAssessmentTDS.SectionDetailsDataTable)Table).FindByWorkID(workId);

            if (row == null)
            {
                throw new Exception("Unavailable row in LiquiForce.LFSLive.BL.CWP.RehabAssessment.RehabAssessmentSectionDetails.GetRow");
            }
