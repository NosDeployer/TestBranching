...
namespace LiquiForce.LFSLive.WebUI.FleetManagement.Common
{
    ...
    public partial class view_edit : System.Web.UI.Page
    {
        ...
        protected FmViewTDS.FmViewConditionTempDataTable fmViewConditionTemp;
        ...
        private bool StepBeginNext()
        {
            ...
            if (Page.IsValid)
            {
                ...
                if (dataSet.LFS_FM_TYPE_VIEW_DISPLAY.Rows.Count <= 0)
                {
                    ...
                    model.LoadByFmType(fmType, companyId);
                    model.UpdateForEdit(viewId, fmType, companyId);
                }
        ...
        private void StepColumnsToDisplayProcessGrid()
        {
            ...
            if (Session["fmTypeViewDisplayDummy"] == null)
            {
                foreach (GridViewRow row in grdColumnsToDisplay.Rows)
                {
                    int displayId = Int32.Parse(grdColumnsToDisplay.DataKeys[row.RowIndex].Values["DisplayID"].ToString());
                    bool selected = ((CheckBox)row.FindControl("cbxSelected")).Checked;
                    int companyId = Int32.Parse(hdfCompanyId.Value);
                    string fmType = hdfFmType.Value;

                    model.Update(fmType, companyId, displayId, selected);
                }
        ...
        protected void grdConditions_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                ...
                int conditionId = Int32.Parse(((DropDownList)grdConditions.Rows[e.RowIndex].Cells[3].FindControl("ddlName")).SelectedValue);
                ...
                int conditionNumber = Int32.Parse(((DropDownList)grdConditions.Rows[e.RowIndex].Cells[2].FindControl("ddlConditionNumber")).SelectedValue);
                ...
                string type = fmTypeViewConditionGateway.GetType(fmType, companyId, conditionId);
                string value = "";
                ...
                int refId = fmViewConditionGateway.GetLastRefIdByViewIdCompanyIdFmTypeConditionId(viewId, companyId, fmType, conditionId);
                refId = refId + 1;
        ...
        protected void grdConditions_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                int conditionId = 0; conditionId = Int32.Parse(((DropDownList)e.Row.FindControl("ddlName")).SelectedValue);
                ...
                if (conditionId > 0)
                {
                    ...
                    if (ddlOperator.SelectedItem == null)
                    {
                        ...
                        fmTypeViewOperatorList.LoadAndAddItem(type, companyId);
                ...
                if (conditionId > 0)
                {
                    ...
                    if ((type == "FixedItems") || (type == "DynamicItems") || (type == "Boolean"))
                    {
                        ...
                        if (type == "FixedItems")
                        {
                            ...
                            fmTypeViewConditionItemList.LoadAndAddItemInView(fmType, companyId, conditionId);
                        ...
                        if (type == "DynamicItems")
                        {
                            ...
                            else
                            {
                                ...
                                fmTypeViewStateList.LoadAndAddItem(companyId);
        ...
        protected void ddlNameNew_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            FmTypeViewConditionGateway fmTypeViewConditionGateway = new FmTypeViewConditionGateway();
            fmTypeViewConditionGateway.LoadByFmTypeConditionId(fmType, companyId, Int32.Parse(ddlName.SelectedValue));
            string type = fmTypeViewConditionGateway.GetType(fmType, companyId, Int32.Parse(ddlName.SelectedValue));
            ...
            if ((type == "FixedItems") || (type == "DynamicItems") || (type == "Boolean"))
            {
                ...
                if (type == "FixedItems")
                {
                    ...
                    // ... For view type
                    FmTypeViewConditionItemList fmTypeViewConditionItemList = new FmTypeViewConditionItemList(new DataSet());
                    fmTypeViewConditionItemList.LoadAndAddItemInView(fmType, companyId, Int32.Parse(ddlName.SelectedValue));
                    ddlValue.DataSource = fmTypeViewConditionItemList.Table;
                    ddlValue.DataValueField = "Name";
                    ddlValue.DataTextField = "Name";
                    ddlValue.DataBind();
        ...
        protected void cvValueData_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            {
                conditionId = Int32.Parse(((DropDownList)grdConditions.FooterRow.FindControl("ddlName")).SelectedValue);
            }
            else
            {
                conditionId = Int32.Parse(((DropDownList)grdConditions.Rows[grdConditions.EditIndex].Cells[3].FindControl("ddlName")).SelectedValue);
            }
            ...
            FmTypeViewConditionGateway fmTypeViewConditionGateway = new FmTypeViewConditionGateway();
            fmTypeViewConditionGateway.LoadByFmTypeConditionId(fmType, companyId, conditionId);
            type = fmTypeViewConditionGateway.GetType(fmType, companyId, conditionId);
        ...
        private void GrdConditionsAdd()
        {
            if (ValidateFooterAdd())
            {
                ...
                if (Page.IsValid)
                {
                    int conditionId = Int32.Parse(((DropDownList)grdConditions.FooterRow.FindControl("ddlName")).SelectedValue);
                    ...
                    int conditionNumber = Int32.Parse(((DropDownList)grdConditions.FooterRow.FindControl("ddlConditionNumber")).SelectedValue);
                    int companyId = Int32.Parse(hdfCompanyId.Value);
                    string fmType = hdfFmType.Value;
                    int viewId = Int32.Parse(hdfViewId.Value);
        ...
        private bool StepLogicNext()
        {
            ...
            if (Page.IsValid)
            {
                ...
                {
                    model.LoadByFmTypeInView(fmType, companyId, true);
                    model.UpdateForEdit(viewId, fmType, companyId);
                }
        ...
        private void StepSortProcessGrid()
        {
            ...
            if (Session["fmTypeViewSortDummy"] == null)
            {
                foreach (GridViewRow row in grdSort.Rows)
                {
                    int sortId = Int32.Parse(grdSort.DataKeys[row.RowIndex].Values["SortID"].ToString());
                    bool selected = ((CheckBox)row.FindControl("cbxSelected")).Checked;
                    int order = 0;

                    try
                    {
                        order = Int32.Parse(((DropDownList)row.FindControl("ddlOrder")).SelectedValue);
                    }
                    catch
                    {
                        order = 0;
                    ...
                    model.Update(fmType, companyId, sortId, order, selected);
                }
        ...
        private void StepSummaryIn()
        {
            ...
            summary = summary + "Conditions: " + modelFmViewConditionNew.GetConditionsForSummary(fmType, companyId) + "\n\n";
