...
function btnViewAddClick()
{
    ...
    {    
        var url = "./../Common/view_add.aspx?source_page=ra_navigator2.aspx&work_type=" + hdfWorkType.value+"&project_id="+ hdfCurrentProjectId.value;
...
function tkrpbLeftMenuItemClick(sender, eventArgs) {
    ...
    switch (item) {
        case "mAddSection":
            window.open("./../Common/project_add_sections.aspx?source_page=ra_navigator2.aspx&project_id=" + hdfCurrentProjectId.value + "&client_id=" + hdfCurrentClientId.value + "&work_type=Rehab Assessment", "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=830, height=715");
