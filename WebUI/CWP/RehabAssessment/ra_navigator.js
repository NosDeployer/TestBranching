...
function btnViewAddClick()
{
    ...
    {    
        var url = "./../Common/view_add.aspx?source_page=ra_navigator.aspx&work_type=" + hdfWorkType.value+"&project_id="+ hdfCurrentProjectId.value;
...
function btnViewEditClick()
{
    ...
    {
        var url = "./../Common/view_edit.aspx?source_page=ra_navigator.aspx&work_type=" + hdfWorkType.value+"&view_id="+ ddlView.value+"&project_id="+ hdfCurrentProjectId.value;
        var remote = window.open( url ,"_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=638, height=450");
        if (remote.opener == null)
        {
            remote.opener = window;
...
function tkrpbLeftMenuItemClick(sender, eventArgs) {
    ...
    switch (item) {
        case "mAddSection":
            window.open("./../Common/project_add_sections.aspx?source_page=ra_navigator.aspx&project_id=" + hdfCurrentProjectId.value + "&client_id=" + hdfCurrentClientId.value + "&work_type=Rehab Assessment", "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=830, height=715");
