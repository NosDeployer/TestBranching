...
namespace LiquiForce.LFSLive.WebUI.CWP.FullLengthLining
{
    ...
    /// fl_wetout_report
    /// </summary>
    public partial class fl_wetout_report : System.Web.UI.Page
    {
        // ////////////////////////////////////////////////////////////////////////
        // EVENTS
        //
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Response.Redirect("./../../error_page.aspx?error=" + "Invalid query string in fl_wetout_report.aspx");
                }
        ...
        {
            mReport1 master = (mReport1)this.Master;
            LiquiForce.LFSLive.BL.CWP.FullLengthLining.FlWetOutReport flWetOutReport = new LiquiForce.LFSLive.BL.CWP.FullLengthLining.FlWetOutReport();
            int companyId = Int32.Parse(hdfCompanyId.Value);
            ...
            if (Page.IsValid)
            {
                ...
                if (ddlClient.SelectedValue == "-1")
                {
                    ...
                    if (cbxSectionId.Checked)
                    {
                        ...
                        flWetOutReport.LoadBySectionId(companyId, sectionsId);
                    }
                }
                // For specific client
                else
                {
                    ...
                    if (ddlProject.SelectedValue == "-1")
                    {
                        if (cbxSectionId.Checked)
                        {
                            ...
                            flWetOutReport.LoadByCompaniesIdSectionId(companyId, int.Parse(ddlClient.SelectedValue), sectionsId);
                        }               
                    }
                    ...
                    else
                    {
                        if (cbxSectionId.Checked)
                        {
                            ...
                            flWetOutReport.LoadByCompaniesIdProjectIdSectionId(companyId, int.Parse(ddlClient.SelectedValue), int.Parse(ddlProject.SelectedValue), sectionsId);
                        }                
                    }
                }
            }
            ...
            master.Data = flWetOutReport.Data;
            master.Table = flWetOutReport.TableName;

            // Get report
            if (flWetOutReport.Table.Rows.Count > 0)
            {
                if (master.Format == "pdf")
                {
                    master.Report = new FlWetOutReport();
                }
                else
                {
                    master.Report = new FlWetOutReportExport();
                }
