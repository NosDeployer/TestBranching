...
var ie = ie4 || ie6;
var ns4 = (agent.indexOf("mozilla/4") >-1 && agent.indexOf("msie") == -1);
var ns6 = (agent.indexOf("mozilla/5") >-1 && agent.indexOf("netscape6") > -1);
var ns7 = (agent.indexOf("mozilla/5") >-1 && agent.indexOf("netscape/7") > -1);
var mozilla = (agent.indexOf("mozilla/5") >-1 && agent.indexOf("netscape6") == -1 && agent.indexOf("netscape/7") == -1 && agent.indexOf("firefox") == -1);
...
var mozillaLowLevel = ns6 || ns7 || mozilla;
...
function _getChildByIdEx(element, id, index, recursive){
	if(element != null){		
		...
		if(recursive){
			for(var i = 0; i < childNodes.length; i ++){
				var foundNode = _getChildByIdEx(childNodes[i], id, index, recursive);
				if(foundNode != null)
					return foundNode;
			}
		}
...
ASPxClientUtils.GetChildByIdEx = function(element, id, index, recursive){
	return _getChildByIdEx(element, id, index, recursive);
}
...
function _checkElementOverflow(curEl, overflow){
	return curEl.style.overflow == overflow || curEl.style.overflowX == overflow || curEl.style.overflowY == overflow || !nsUpLevel && (curEl.currentStyle.overflow == overflow || curEl.currentStyle.overflowX == overflow || curEl.currentStyle.overflowY == overflow);
}
...
function _isStaticElement(curEl){
	return !(curEl.style.position == "absolute" || curEl.style.position == "relative" || !nsUpLevel && (curEl.currentStyle.position == "absolute" || curEl.currentStyle.position == "relative") || _isScrollableElement(curEl));
}

function _isTerminalElement(curEl){
	return _isScrollableElement(curEl) && (curEl.style.width != "" || curEl.style.height != "" || !nsUpLevel && (curEl.currentStyle.width != "" || curEl.currentStyle.height != ""));
}
...
function _absoluteX(curEl, ownerElementId, correctPos, correctionNeeded, checkTermEl, addElOffset){
    ...
    if(curEl != null){
		if(curEl.offsetParent != null){		
			...
			if(addElOffset && ((curEl.clientWidth != 0 || nsUpLevel) && (!correctPos || _isStaticElement(curEl))))
				posX += curEl.offsetLeft + (!nsUpLevel && curEl.tagName.toLowerCase() != "table" ? curEl.clientLeft : 0);
...
function _absoluteY(curEl, ownerElementId, correctPos, correctionNeeded, checkTermEl, addElOffset){
    ...
    if(curEl.offsetParent != null){
		...
		if(addElOffset && ((curEl.clientHeight != 0 || nsUpLevel) && (!correctPos || _isStaticElement(curEl))))
			posY += curEl.offsetTop + (!nsUpLevel && curEl.tagName.toLowerCase() != "table" ? curEl.clientTop : 0);
		if((!correctPos || !checkTermEl))
...
function _getDocumentScrollLeft(){
	if(nsUpLevel) return window.pageXOffset;
	else return _getScrollLeft(document.body);
}
...
function _getDocumentScrollTop(){
	if(nsUpLevel) return window.pageYOffset;
	else return _getScrollTop(document.body);
}
...
ASPxClientUtils.GetEvent = function(){
	return _getEvent();
}
...
ASPxClientUtils.GetEventSource = function(){
	return _getEventSource();
}

function _getMouseButton(evt){
	if(nsUpLevel) return evt.which;
...
function _getScrollBarWidth(){
	if(scrollBarWidth == -1){
		...
		div.style.cssText = "overflow:auto;height:100;width:100;visibility:hidden";
		div.innerHTML = "<div style=\"height:400;width:400\"></div>";
		document.body.appendChild(div);
		scrollBarWidth = div.offsetWidth - div.clientWidth + (mozilla && mozillaVersion < mozillaCorrectScrollBarVersion ? 15 : 0);
...
function _getUnitConverter(){
	if(unitConverter == null){
		...
		unitConverter.style.cssText = "visibility:hidden";
...
function ASPxPostBack(){
	...
	this.Initialize = function(evt, eventTarget, eventArgument, sender, isCallBack){
...
function __initASPxPostBack(evt, eventTarget, eventArgument, sender, isCallBack){
	__ASPxPostBack.Initialize(evt, eventTarget, eventArgument, sender, isCallBack);
}
...
function ASPxClientWebControl(name){
	...
	this.InitPostBack = function(evt, eventArgument){
		__initASPxPostBack(evt, this.name, eventArgument, this, false);
	}	
...
function GetWebControlCollection(){
	if(__ASPxWebControlCollection == null){
		__ASPxWebControlCollection = new ASPxClientCollection();
	}
...
indicatorImgZIndex = 9000;
popupZIndex = 10000;
dragHdrZIndex = 100000;