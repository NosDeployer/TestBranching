...
function getDragHeaderLeft(header, gridName, isGrouped, borderCollapse){
	...
	if(nsUpLevel) return absX + (mozilla ? (isGrouped || borderCollapse ? 0 : 1) : 0);
...
function getDragHeaderTop(header, gridName, isGrouped, borderCollapse){
	...
	if(nsUpLevel) return absY + (mozilla ? (isGrouped || borderCollapse ? 0 : 1) : 0);
	else return absY + (isGrouped && borderCollapse ? 0 : 0);
}

function getDragHeaderWidth(header, isGrouped, borderCollapse){
	if(nsUpLevel) return _getElementWidth(header) - (mozilla ? (isGrouped ? 1 : (borderCollapse ? 1 : 2)) : 2);
...
function getDragHeaderHeight(header, isGrouped, borderCollapse){
	if(nsUpLevel) return _getElementHeight(header) - (mozilla ? (isGrouped ? 1 : (borderCollapse ? 1 : 2)) : 2);
...
function startDragHeader(evt, gridName, header, hdrInd, isGrouped, borderCollapse){
	...
	if(header != null){
		...
		drgHdr.style.visibility = "hidden";	
		drgHdr.onselectstart = new Function("return false");
		...
		curColInd = hdrInd;
		curX = evt.clientX + _getDocumentScrollLeft();
		curY = evt.clientY + _getDocumentScrollTop();		
		curDragEl = drgHdr;
...
function getResizingCursorPosition(evt){
	return evt.clientX - (ie && isRtl() ? _getScrollBarWidth() + document.body.scrollWidth - document.body.clientWidth : 0);
}
...
function startColumnSizing(evt, gridName, gridObj, header){
	...
	if(curSzLn != null){
		curGridName = gridName;		
		curDragAct = 10000;		
		curX = (getResizingCursorPosition(evt) + _getDocumentScrollLeft()) - _absoluteX(curSzLn.offsetParent, gridName, false, true, true, true);
			...
			if(_indexOf != -1){
				...
				if(gridObj != null){					
					...
					if(!isGroupHeader(header)){						
						var clientX = getResizingCursorPosition(evt) + _getDocumentScrollLeft();
						curRszHdr = gridObj.htmlRender.GetResizingHeaderInfo(header, hdrInd, clientX);
						if(curRszHdr != null){
...
function OnDMMove(evt){			
	...
	if(curDragEl != null){		
		if(_isLeftMouseButtonPressed(evt)){
			...
			var newX = evt.clientX + _getDocumentScrollLeft();
			var newY = evt.clientY + _getDocumentScrollTop();
			var distX = newX - curX;
			var distY = newY - curY;
			...
			if(Math.abs(distX) > threshold || Math.abs(distY) > threshold){
				if(curEnblHdrDrag){
					...
					if(curDragAct == 10001)
						curDragAct = 3;
					newX = evt.clientX + _getDocumentScrollLeft();
					newY = evt.clientY + _getDocumentScrollTop();		
		...
		if(_isLeftMouseButtonPressed(evt)){
			newX = (getResizingCursorPosition(evt) + _getDocumentScrollLeft()) - _absoluteX(curSzLn.offsetParent, curGridName, false, true, true, true) - 2;
...
function AfterScroll(gridName){	
	var scrollLeftInput = _getHiddenInput(gridName + "ScrollLeft");
	if(scrollLeftInput != null)
		scrollLeftInput.value = _getElementById(gridName + "scroller").scrollLeft;
	var scrollTopInput = _getHiddenInput(gridName + "ScrollTop");
	if(scrollTopInput != null)
		scrollTopInput.value = _getElementById(gridName + "scroller").scrollTop;
}
