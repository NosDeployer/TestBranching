...
function ASPxClientEditorBase(name, dataControllerName, fieldIndex){
	...
	this.GetEditValue = function(){
		this.CheckInitialized();
		if(this.activeElement != null)
			return this.CanEditValue() ? this.GetEditorValue() : GetViewerValue();
		return null;
	}
	...
	this.InitPostBack = function(evt, eventArgument){
		__initASPxPostBack(evt, this.GetUniqueName(), eventArgument, this, false);
	}
...
function OnECancelChanges(name){	
	var editor = GetEditorCollection().Get(name);
	if(editor != null) editor.OnCancelChanges();	
}
...
function GetEditorCollection(){
	if(__ASPxEditorCollection == null){
		__ASPxEditorCollection = new ASPxClientCollection();
	}
