...
function GetLookAndFeelScrollBarCollection(){
	if(__ASPxLookAndFeelScrollBarCollection == null){
		__ASPxLookAndFeelScrollBarCollection = new ASPxClientWebCollection();
	}
...
function OnLFSBNativeScroll(name, element){
	...
	if(LFScrollBar != null){
		...
		if(pos != LFScrollBar.position){
			...
			var component = element.componentFromPoint(_getEventX(evt), _getEventY(evt));
			switch(component){
				case "scrollbarDown":
				case "scrollbarRight":
...
function OnLFSBStartDragging(e, scrollBar){
	...
	draggableLFScrollBarX = _getEventX(e);
	draggableLFScrollBarY = _getEventY(e);
}

function OnLFSBDragging(e, scrollBar){
	var pos = scrollBar.horizontal ? _getEventX(e) : _getEventY(e);
