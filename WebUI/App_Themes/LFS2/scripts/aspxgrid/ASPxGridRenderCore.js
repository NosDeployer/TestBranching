function ASPxGridRenderCore(grid){
	...
	this.ShowSelection = function(tr, rowIndex){	
		...
		if(rowLevel < this.grid.groupColumns.length){
			...
			if(!this.grid.OnCustomRenderSelection(td, this.grid.groupColumns[rowLevel], true, true)){
				td.style.color = _encodeColor(this.selectedForeColor);
				td.style.backgroundColor = _encodeColor(this.selectedBackColor);
			}
			tr.selectionMode = "GR";
		}
		else{					
	...
	this.CorrectScrollersPos = function(tr){
		if(this.IsScrollBarShown()){
			if(!this.nativeScrolling){
				if(this.IsVertScrollBarShown() && !(this.IsClientSideScrollBars() && this.grid.GetPageSize() > 0 && !opera)){
	...
	this.IsScrollBarShown = function(){
		return !(this.scrollBars == "None" || (this.scrollBars == "Auto" && (!this.IsClientSideScrollBars() || opera)));
	}

	this.IsVertScrollBarShown = function(){
		return (this.scrollBars == "Vertical") || (this.scrollBars == "Both") ||
			(this.scrollBars == "Auto" && this.IsClientSideScrollBars() && !opera) || 
			(this.IsScrollBarShown() && !ie);
	}
	...
	this.GetAbsoluteX = function(element){
		if((safari || opera) && _checkTagName(element, "TR")){
			var tableElement = _getParentByTagName(element, "TABLE");
			if(tableElement != null)
				return _absoluteX(tableElement, this.grid.name, false, false, false, true);
		}
		return _absoluteX(element, this.grid.name, false, false, false, true);
	}
	this.GetAbsoluteY = function(element){
		if((safari || opera) && _checkTagName(element, "TR")){
			var tableElement = _getParentByTagName(element, "TABLE");
			if(tableElement != null && element == tableElement.rows[0])
				return _absoluteY(tableElement, this.grid.name, false, false, false, true);
			else{
				var cellElement = _getChildByTagName(element, "TD", 0);
				if(cellElement != null)
					return _absoluteY(cellElement, this.grid.name, false, false, false, true);
			}
		}
		return _absoluteY(element, this.grid.name, false, false, false, true);
	}
	this.GetTargetHeaderInfo = function(x, y, currentColIndex){
		var headerpanel = this.GetHtmlHeaderPanel();	
		if(headerpanel != null){
			var headerpanelX = this.GetAbsoluteX(headerpanel);
			var headerpanelY = this.GetAbsoluteY(headerpanel);
			var headerpanelRight = headerpanelX + _getElementWidth(headerpanel);
			var headerpanelBottom = headerpanelY + _getElementHeight(headerpanel);
			if(x > headerpanelX && x < headerpanelRight && y > headerpanelY && y < headerpanelBottom){
				var targetIndex = 0;			
				var headerIndex = 0;			
				for(var i = 0; i < this.grid.internalColumns.length; i ++){
					...
					if(column.type != "" && column.type != "GroupingIndentColumn"){
						...
						if(header != null){
							var headerX = this.GetAbsoluteX(header);
							...
							if(x > headerX && x < headerX + headerWidth){
								...
								if((column.type == "BoundColumn" || column.type == "TemplateColumn") && (scroller == null || scroller.scrollLeft == null || targetX >= scroller.scrollLeft && targetX <= scroller.scrollLeft + _getElementWidth(scroller))){
									...
									targetHeaderInfo.x = this.GetAbsoluteX(header) + (before ? 0 : headerWidth);
									targetHeaderInfo.y = this.GetAbsoluteY(header);
	...
	this.GetTargetGroupedHeaderInfo = function(x, y, currentColIndex){	
		...
		if(grouppanel != null){		
			var grouppanelX = this.GetAbsoluteX(grouppanel);
			var grouppanelY = this.GetAbsoluteY(grouppanel);
			var grouppanelRight = grouppanelX + _getElementWidth(grouppanel);
			var grouppanelBottom = grouppanelY + _getElementHeight(grouppanel);
			if(safari){
				var headerpanel = this.GetHtmlHeaderPanel();	
				if(headerpanel != null){
					var headerpanelY = this.GetAbsoluteY(headerpanel);
					grouppanelBottom = headerpanelY;
				}
			}
			if(x > grouppanelX && x < grouppanelRight && y > grouppanelY && y < grouppanelBottom){
				if(this.grid.groupColumns.length == 0){
					var targetHeaderInfo = new Object();
					targetHeaderInfo.x = grouppanelX;
					targetHeaderInfo.y = grouppanelY;
					targetHeaderInfo.height = grouppanelBottom - grouppanelY;
	...
	this.GetResizingHeaderInfo = function(header, columnIndex, x){
		var headerAbsoluteX = this.GetAbsoluteX(header);	
		...
		if((leftEdge < (x + columnSizingEdge) && leftEdge > (x - columnSizingEdge)) || (rightEdge < (x + columnSizingEdge) && rightEdge > (x - columnSizingEdge))){
			...
			if(columnIndex >= 0){
				...
				if(column != null && column.enableColumnSizing){
					...
					resizingHeaderInfo.x = this.GetAbsoluteX(resizingHeader);
	...
	this.GetResizedColumnWidth = function(columnIndex, correctBorderWidth){
		var value = this.grid.newColumnWidths[columnIndex];
		value -= 2 * this.grid.cellSpacing;
		if(correctBorderWidth)
			value -= (this.showGridLinesBetweenButtons && !this.borderCollapse ? 2 : 1);
		return new ASPxClientUnit(value, "px");
	}
	...
	this.SetGroupPanelWidth = function(width){
		if(this.showGroupPanel){
			var groupPanelCell = _getFirstChild(this.GetHtmlGroupPanel());
			_getFirstChild(groupPanelCell).style.width = width;
		}
	}
	...
	this.SetCellsWidth = function(synchronizeColumnsWidth){
				...
				if(_strIndexOf(tr.id, getPreviewRowMark()) < 0){
					...
					for(var j = 0; j < this.grid.internalColumns.length; j ++){
						...
						if((column.type == "BoundColumn" || column.type == "TemplateColumn")){
							...
							if(!this.grid.directRender || (editMode && !column.readOnly)){
								var editor = column.GetEditor();
								if(!this.UseItemTemplateEx(column, editMode) && editor != null)
									editor.SetEditorSize(td.firstChild, editMode, newWidth.ConvertToPx().Inc(- 2 * this.grid.cellPadding), null);
								else
		...
		if(synchronizeColumnsWidth){
			for(var i = 0; i < this.grid.columns.length; i ++){
				...
				var index = this.grid.GetColumnInternalIndex(column);
				if(index > -1)
					column.width = this.GetResizedColumnWidth(index, false);
			}
