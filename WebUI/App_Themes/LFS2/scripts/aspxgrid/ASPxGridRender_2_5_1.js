...
function ASPxGridRender(grid){
	...
	this.CreateDataTDContent = function(td, column, isOdd){		
		...
		if(!handled && itemTemplate == null && column.GetEditor() != null){
			if(!this.grid.directRender){
				width = column.GetCellWidth();
				if(height.type == "%")
					height = ASPxClientUnit.GetHundredPercentUnit();
				column.GetEditor().ShowEditor(td, false, isOdd, width, height);
			}			
		}
	...
	this.CreateEditTDContent = function(td, column, isOdd){
		...
		if(!handled && editItemTemplate == null && column.GetEditor() != null){
			width = column.GetCellWidth();
			if(height.type == "%")
				height = ASPxClientUnit.GetHundredPercentUnit();
			column.GetEditor().ShowEditor(td, !this.grid.IsColumnReadOnly(column), isOdd, width, height);
		}
		if(this.FireCustomRenderAfterDefaultRender())
	...
	this.CreatePreviewTDContent = function(td, width, height){
		...
		if(!handled && previewTemplate == null){
			if(!this.grid.directRender){
				...
				td.firstChild.style.cursor = "default";
				td.firstChild.style.overflow = "hidden";
				if(!height.IsEmpty() && height.type == "px")
					td.firstChild.style.height = height.ToString();
				td.firstChild.style.width = width.ToString();
				td.firstChild.style.paddingLeft = horzTextPadding;
				td.firstChild.style.paddingRight = horzTextPadding;
			}
		}
		if(this.FireCustomRenderAfterDefaultRender())
	...
	this.InitializeRowBtnTDContent = function(td, column, rowButton, inHeaderPanel, width, height, imageUrl, enabled, toolTip, initializeDefault){	
		...
		if(this.instantiateTemplatesInButtons || !this.CustomBindRowBtn(td, column, rowButton, inHeaderPanel, width, height, imageUrl, enabled)){
			...
			if(!customBind && initializeDefault)
				InitializeLFButton(td, "", registerId, this.grid.lookAndFeel, true, null, null, imageUrl, "", "", "", enabled, toolTip, null, ASPxClientUnit.GetHundredPercentUnit(), nsUpLevel ? height : ASPxClientUnit.GetHundredPercentUnit());
		}
	}
	...
	this.CreateGroupingTDContent = function(td, column, width, height){	
		...
		if(!handled && groupItemTemplate == null){
			if(!this.grid.directRender){
				td.appendChild(document.createElement("DIV"));
				td.firstChild.className = this.GetClassName("C" + column.index + "GIV");
				td.firstChild.style.cursor = "default";	
				td.firstChild.style.width = this.grid.GetGroupItemTextSectionWidth(column.groupIndex).ToString();
			}
		}
		if(this.FireCustomRenderAfterDefaultRender())
	...
	this.GetRowBtnOnClick = function(rowBtnType){
		...
		return new Function("OnRBCl(event, \"" + rowBtnType + "\")")
	}
	...
	this.CreateTRContent = function (tr, row, preview, createRowButtons){		
		...
		if(this.disableSelection && (!isEditRow || preview))
			tr.onselectstart = new Function("return OnTRSS(event);");
		else
	...
	this.GetGroupingText = function(row, groupingColumn){
		...
		var valueText = groupingColumn.GetEditor() != null ? groupingColumn.GetEditor().GetDisplayTextByValue(groupingValue) : FormatValue(groupingValue, "", groupingColumn.GetDataType());
		var summaryText = "";
	...
	this.CorrectGroupTRBorders = function(groupingTd, row, isLastTR, isExpandBtnCell){
		if(this.correctGroupItemBorders){
			if(isExpandBtnCell){
				if(this.borderCollapse && !nsUpLevel && this.NeedCorrectGroupTRBottonBorder(row))
	...
	this.FillDataRowTD = function(td, column, row, cellValue){
		...
		if(!handled && column.itemTemplate == null && column.GetEditor() != null){
			if(!this.grid.directRender)
				column.GetEditor().BindEditor(td.firstChild, row.GetDataControllerRow(), cellValue.value, false, cellValue.renderValueDirectly);
			else
				_setElementInnerText(td, cellValue.renderValueDirectly ? cellValue.value.toString() : column.GetEditor().GetDisplayTextByValue(cellValue.value));
		}
	}
	...
	this.FillEditRowTD = function(td, column, row, cellValue){
		...
		if(!handled && column.editItemTemplate == null && column.GetEditor() != null)
			column.GetEditor().BindEditor(td.firstChild, row.dataRow, cellValue.value, !this.grid.IsColumnReadOnly(column), cellValue.renderValueDirectly);
	}
	...
	this.RetrieveEditValue = function(td, column, row){
		var handled = false;
		...
		if(handled && column.fieldIndex >= 0)
			row.GetDataControllerRow().SetValue(column.fieldIndex, ParseValue(eventArgs.value, column.editFormatString, column.GetDataType()));
		return handled;
	}

	this.RetrieveEditValues = function (){	
		var focusedRow = this.grid.GetFocusedRow();
		var tr = this.GetTRByPos(this.GetHtmlRows(), this.grid.focusedPos);
		if(focusedRow != null && tr != null){
			var firstColumnOffset = this.grid.GetLeftRowBtnColumnsOffset();
			var lastColumnOffset = this.grid.GetRightRowBtnColumnsOffset();
			for(var i = firstColumnOffset; i <= lastColumnOffset; i ++){		
				var column = this.grid.internalColumns[i];
				if(column.type == "BoundColumn" || column.type == "TemplateColumn")
					this.RetrieveEditValue(tr.cells[i], column, focusedRow);
			}
		}
	}

	this.RetrieveColumnEditValue = function(column){
		var focusedRow = this.grid.GetFocusedRow();
		var tr = this.GetTRByPos(this.GetHtmlRows(), this.grid.focusedPos);
		if((column.type == "BoundColumn" || column.type == "TemplateColumn") && focusedRow != null && tr != null)
			return this.RetrieveEditValue(tr.cells[this.grid.GetColumnInternalIndex(column)], column, focusedRow);
		return false;
	}			
	...
	this.UpdateTotalWidth = function(){
		...
		if(nsUpLevel && this.IsScrollBarShown() && !this.IsHorzScrollBarShown() && width.type != "%")
	...
	this.ColumnsRecreated = function(){
		...
		if(this.IsVertScrollBarShown() && !this.nativeScrolling){
			...
			if(vertScrollArea != null)
				vertScrollArea.parentNode.colSpan = this.grid.internalColumns.length;
		}
	...
	this.CreateTitleTDContent = function (td, width, height, text){		
		...
		if(!handled && titleTemplate == null)
			this.CreatePanelContents(td, text, width, this.titleBordersWidth);
		if(this.FireCustomRenderAfterDefaultRender())
	...
	this.CreatePanelContents = function(td, text, width, bordersWidth){
	...
	this.CreateGroupPanelTDContent = function (td, width, height, text){		
		...
		if(!handled && groupPanelTemplate == null)
			this.CreatePanelContents(td, text, width, this.groupPanelBordersWidth);
		if(this.FireCustomRenderAfterDefaultRender())
	...
	this.InvalidateGroupPanel = function (){		
		...
		if(groupPanel != null){		
			...
			if(this.grid.groupColumns.length > 0){	
				this.CreatePanelContents(groupPanelFirstChild, "", this.grid.GetTotalWidth(true), this.groupPanelBordersWidth);
		...
		if(items != null){			
			...
			if(this.focusedTR != null){
				...
				if(this.grid.IsEditMode() && this.grid.GetFocusedColumn() != null){
					...
					if(!handled && focusedColumn.GetEditor() != null && __getFocusedControl() == this.grid)
						focusedColumn.GetEditor().FocusEditor();		
				}
			}
		}	
	}
	...
	this.ResetRegularVertScrollBar = function(){
		...
		if(scrollBar != null)
			scrollBar.SetScrollableID(this.grid.name + (this.grid.GetPageSize() > 0 ? "vertscroller" : "vertscrollarea"));
	}
	...
	this.ResetNativeVertScrollBar = function(){
		if(nsUpLevel){
			var scroller = this.GetHtmlScroller();
			if(scroller != null)
				scroller.style.overflow = this.grid.GetPageSize() == 0 || this.IsHorzScrollBarShown() ? "auto" : "visible";
		}
		var vertScroller = this.GetHtmlVertScroller();
		if(vertScroller != null)
			vertScroller.style.width = (this.grid.GetPageSize() > 0 && (!nsUpLevel || !this.IsHorzScrollBarShown()) ? (_getScrollBarWidth() + 1) : 0);
	}
