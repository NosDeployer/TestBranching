...
function tkrmTopItemClicking(sender, args) {
    ...
    switch (itemValue) {
        case "mPreview":
            var hdfCostingSheetId = document.getElementById(hdfCostingSheetIdId);
            var hdfProjectId = document.getElementById(hdfProjectIdId);

            window.open("./project_combined_costing_sheets_preview.aspx?type=detailed&costing_sheet_id=" + hdfCostingSheetId.value + "&project_id=" + hdfProjectId.value, "_blank", "toolbar=yes, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=no, width=800, height=680");
