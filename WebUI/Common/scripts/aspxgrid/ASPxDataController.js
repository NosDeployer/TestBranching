...
function ASPxClientDataController(name){	
		...
		if(autoIncCurrent == 0){
			...
			for(var i = 0; i < this.rows.length; i ++){
				...
				if(autoIncCurrent < rowValue)
					autoIncCurrent = Math.floor(rowValue);
			}
		}
		autoIncCurrent ++;
	...
	this.EndDataBatchUpdate = function(){
		if(this._lockDataChangesSync == 0) return;
	...
	this.AfterPostBack = function(){
		var input = _getHiddenInput(this.name + "InsertedRows");
		if(input != null) input.value = "";
		input = _getHiddenInput(this.name + "EditedRows");
		if(input != null) input.value = "";
		input = _getHiddenInput(this.name + "DeletedRows");
		if(input != null) input.value = "";
	}
...
ASPxClientDataControllerRow.SetValue = function(index, value){
	if(index >= 0 && index < this.values.length){
		var column = this.owner.columns[index];
		if(this.owner.IsColumnReadOnly(column)){
			alert("The value of the '" + column.name + "' column cannot be changed because it is readonly.");
			return;
		}
		if(!this.owner.IsEditMode() || this != this.owner.GetFocusedRow()){
			alert("The value of the '" + column.name + "' column cannot be changed in browse mode.");
			return;
		}
...
function GetDataControllerCollection(){
	if(__ASPxDataControllerCollection == null){
		__ASPxDataControllerCollection = new ASPxClientWebCollection();
	}
...
window.onload = function(){
	if(DataControllerFirstLoad){
		savedDataControllerFormOnBeforeUnload = window.onbeforeunload;
		window.onbeforeunload = DataControllerFormOnBeforeUnload;
		if(_getServerForm() != null){
			savedDataControllerFormOnSubmit = _getServerForm().onsubmit;
			_getServerForm().onsubmit = DataControllerFormOnSubmit;
		}
...
function DataControllerFormOnSubmit(){
	isSubmit = true;
	if(_isExists(savedDataControllerFormOnSubmit))
		return savedDataControllerFormOnSubmit();
}

function DataControllerFormOnBeforeUnload(){
	if(!isSubmit){
	...
	isSubmit = false;
	if(_isExists(savedDataControllerFormOnBeforeUnload))
		return savedDataControllerFormOnBeforeUnload();
}