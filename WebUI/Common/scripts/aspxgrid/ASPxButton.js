...
function ASPxClientButton(name){
	...
	this.IsNative = function(){
		var element = this.GetRoolElement();
		return _checkTagName(element, "INPUT");
	}
	...
	this.InitPostBack = function(evt, eventArgument){
		__initASPxPostBack(evt, this.name, eventArgument, this, false, true);
	}
	this.Focus = function(){
		this.FocusControl();
	}
...
function ASPxButtonCollection(){
	...
	this.CanDefaultButtonAction = function(){
		...
		ret = ret && (document.activeElement != null) && !_checkTagName(document.activeElement, "BUTTON") &&
			(!_checkTagName(document.activeElement, "INPUT") || array_indexOf(inputTypes, document.activeElement.type) == -1);
