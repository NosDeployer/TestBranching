...
function ASPxClientGrid(name, dataControllerName){
	...
	this._lockRowsSynchronization = false;		
			...
			switch(processingMode){
				case "Client":
					...
					if(this.IsEditMode() && this.GetFocusedColumn() != null){
						...
						if((!this.htmlRender.initialized || !this.htmlRender.RetrieveColumnEditValue(focusedColumn)) && focusedColumn.GetEditor() != null)
							canSet = this.GetFocusedColumn().GetEditor().InterimStoreEditValue();
					}
					if(canSet){
	...
	this.SetFirstVisibleRowInternal = function (row){	
		if(this.fixedPageNavigation){
			var pageIndex = Math.floor(this.GetVisibleIndexByRowIndex(row.index) / this.pageSize);
			if(pageIndex >= 0)
	...
	this.ProcessSelectionInternal = function (oldFocusedRow, newFocusedRow, changeSelection, clearPrevSelection, selectRange){	
		if(this.multiSelection){
			this._lockRowsSynchronization = true;
	...
	this.SearchRow = function (column, val, startRow, convertToString, partialSearch, ignoreCase, searchForward, findNext, cycle){
		if(column != null && this.ClearCurrentState()){
			...
			var row = this.dataController.SearchRowInternal(column.GetDataControllerColumn(), val, startRow != null ? startRow.GetDataControllerRow() : null, convertToString, partialSearch, ignoreCase, searchForward, findNext, cycle, column.GetEditor().dataFormatString);
			if(row != null)
	...
	this.StoreEditValue = function(){
		if(this.htmlRender.initialized)
			this.htmlRender.RetrieveEditValues();
	}
	this.ValidateEditValue = function(){
		...
		for(var i = 0, columnCount = this.internalColumns.length; i < columnCount; i ++){
			...
			if(column.type == "BoundColumn" && column.GetEditor() != null){
				var cancel = false;
				if(!this.ValidateColumnValue.IsEmpty()){
					var args = new ASPxClientValidateColumnValueEventArgs(row, column, column.GetEditor().GetEditValue());
					this.ValidateColumnValue.FireEvent(this, args);
					column.GetEditor().SetEditValue(args.value);
					cancel = args.cancel;
				}
				if(cancel){
					if(args.message != "")
	...
	this.DataCtrlrRaiseAfterDelete = function(dataRow){
		this.OnAfterCommand(this.AfterDelete, this.GetRowByDataRow(dataRow));
	}
	...
	this.CanCompareRows = function(){
		return true;
//		return !this.CompareRows.IsEmpty();
	}
	...
	this.SetColumnWidthByUI = function(column, newWidth){
		...
		switch(processingMode){
			case "Client":
				...
				this.OnColumnWidthChanged(this);
				break;
			case "Server":
				this.SendPostBack(false);
				break;
	...
	this.GetTotalColumnsWidth = function (includeSpecColumns){
		...
		var totalColumnsWidth = new ASPxClientUnit(columnsWidth, "px");
	...
	this.GetPreviewSectionWidth = function(){
		return this.GetTotalColumnsWidth(false).ConvertToPx().Inc(-1 - (nsUpLevel ? 2 * horzTextPadding : 0));
	}
	this.GetGroupItemTextSectionWidth = function(groupLevel){
		return this.GetTotalColumnsWidth(false).ConvertToPx().Add(this.groupRowIndent.ConvertToPx().Mul(this.groupColumns.length - groupLevel - 1)).Inc(-1 - (nsUpLevel ? 2 * horzTextPadding : 0));
	}
	...
	this.DeactivateEditors = function(){
		for(var i = 0, columnCount = this.internalColumns.length; i < columnCount; i ++){
			if(this.internalColumns[i].type == "BoundColumn" && this.internalColumns[i].GetEditor() != null)
				this.internalColumns[i].GetEditor().DeactivateEditor();
		}
	}
	...
	this.SynchronizeRows = function (){
		if(!this._lockRowsSynchronization){
	...
	this.ClearPostBackEvtSrcType = function(){
		this.postBackInfo.evtSrcType = "";
	}
}
	...
	this.SetWidth = function (newWidth){
		if(!newWidth.Equals(this.width)){			
			...
			if(this.owner.IsEditMode() && this.owner.GetFocusedColumn() == this){
				if((!this.owner.htmlRender.initialized || !this.owner.htmlRender.RetrieveColumnEditValue(this)) && this.GetEditor() != null)
					this.GetEditor().InterimStoreEditValue();
			}
