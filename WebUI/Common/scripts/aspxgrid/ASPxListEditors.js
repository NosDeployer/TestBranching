...
function ASPxClientListItem(value, displayText, imageUrl){
	this.value = value; 
...
function ASPxClientListEditBase(name, dataControllerName, fieldIndex){
	...
	this.AddItem = function(value, displayText, imageUrl){
		this.items.push(new ASPxClientListItem(value, displayText, imageUrl));
	}
...
function ASPxClientListBox(name, dataControllerName, fieldIndex){
	...
	this.IsNative = function() {
		return _checkTagName(this.activeElement, "SELECT");
	}
	...
	this.lebaseSetEditorSize = this.SetEditorSize;
	this.SetEditorSize = function(element, editMode, width, height){
		this.lebaseSetEditorSize(element, editMode, width, height);
		if(editMode && !this.IsNative() && !this.dropDown)
			InitializeLFScrollBoxSize(element, width, height);
	}
	this.lebaseSetEditorReadOnly = this.SetEditorReadOnly;
	this.SetEditorReadOnly = function(value){
		this.lebaseSetEditorReadOnly(value);
		if(!this.IsNative()){
			var LFListBox = this.GetLFListBox();
	...
	this.lebaseSetEnabled = this.SetEnabled;
	this.SetEnabled = function(enabled){
		this.lebaseSetEnabled(enabled);
		var LFListBox = this.GetLFListBox();
	...
	this.lebaseOnEditorKeyDown = this.OnEditorKeyDown;
	this.OnEditorKeyDown = function(e){
		...
		return this.lebaseOnEditorKeyDown(e);
	}
	this.lebaseOnEditorKeyUp = this.OnEditorKeyUp;
	this.OnEditorKeyUp = function(e){
		...
		return this.lebaseOnEditorKeyUp(e);
	}
	...
	this.lebaseInitializeControl = this.InitializeControl;
	this.InitializeControl = function(){
		this.lebaseInitializeControl();
	...
	this.Remove = function(value){
		...
		if(valueIndex > -1){
			...
			array_removeAt(this.items, valueIndex);
	...
	this.AfterPostBack = function(){	
		var inputElement = _getHiddenInput(this.name + "InsertedItems");
		if(inputElement != null) inputElement.value = "";
		var inputElement = _getHiddenInput(this.name + "DeletedItems");
		if(inputElement != null) inputElement.value = "";
	}
...
function ASPxClientDropDownList(name, dataControllerName, fieldIndex){
	...
	this.lbbaseBindEditValue = this.BindEditValue;
	this.BindEditValue = function(value, renderValueDirectly){
		this.lbbaseBindEditValue(value, renderValueDirectly);
	...
	this.lbbaseDeactivateEditor = this.DeactivateEditor;		
	this.DeactivateEditor = function(){
		if(this.activeElement == null) return;
		
		this.HidePopup();
		this.lbbaseDeactivateEditor();	
	}

	this.lbbaseSetEditorReadOnly = this.SetEditorReadOnly;
	this.SetEditorReadOnly = function(value){
		this.lbbaseSetEditorReadOnly(value);
		if(this.dropDownEditable && !this.IsNative()){
			var textElement = this.GetTextInputElement();
			if(textElement != null) textElement.readOnly = value;
		}
	}
	this.lbbaseSetEnabled = this.SetEnabled;
	this.SetEnabled = function(enabled){
		this.lbbaseSetEnabled(enabled);
		var LFPopup = this.GetLFPopup();
	...
	this.lbbaseInitializeControl = this.InitializeControl;
	this.InitializeControl = function(){
		this.CheckPopupsInitialized();
		this.lbbaseInitializeControl();
	}
		
	this.lbbaseOnEditorKeyDown = this.OnEditorKeyDown;
	this.OnEditorKeyDown = function(e){
		var evt = _getEvent(e);
		var LFPopup = this.GetLFPopup();
		if(LFPopup != null){
		...
		return this.lbbaseOnEditorKeyDown(e);
	}
...
function ASPxClientRadioButtonList(name, dataControllerName, fieldIndex){
	...
	this.OnCancelChanges = ChangeCancel = function(element){
		if(!this.editable){
