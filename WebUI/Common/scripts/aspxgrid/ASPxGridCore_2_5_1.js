...
function ASPxClientGridCore(name){
	...
	this.InitializeHScrollerPos = function(pos){	
		var scroller = _getElementById(this.name + "scroller");
	...
	this.GetColumnIndexByTDId = function (tdId){
		if(tdId != "")
			return tdId >= 0 ? this.internalColumns[tdId].index : tdId;
		return -1;
	}			
	...
	this.GetPreviewWidth = function(){
		...
		return previewWidth - 1 - (nsUpLevel ? 2 * horzTextPadding : 0);
	}
	...
	this.GetGroupItemTextCellWidth = function(){
		...
		return groupItemTextCellWidth - 1 - (nsUpLevel ? 2 * horzTextPadding : 0);
	}
	...
	this.PostEditCore = function(evt, evtSrcType, rowId, assistPar, processOnServer){
		this.InitPostBack(evt, "POSTEDIT",  evtSrcType, "0", assistPar);
	...
	this.CancelEditCore = function(evt, evtSrcType, rowId, assistPar, processOnServer){
		this.InitPostBack(evt, "CANCELEDIT",  evtSrcType, "0", assistPar);
		if(!processOnServer)					
			this.CancelByUI();
		else
			this.SendPostBack(false);
	...
	this.InitPostBack = function(evt, actionID, sourceType, activeElement, assistElement){
		...
		__initASPxPostBack(evt, this.name, actionID + argsDlmtr + sourceType + argsDlmtr + activeElement + argsDlmtr + assistElement, this, this.enableCallBacks);
	}
	...
	this.OnHeaderDragEndCore = function(evt, action, columnIndex, targetIndex){		
		...
		if(this.clientSideMode){
			...
			processingMode = this.OnHeaderDragEnd(column, groupIndex, visibleIndex, false);
		}
		else{
	...
	this.OnRowButtonClickCore = function(evt, action, rowId){
		...
		if(processingMode != "Handled"){
			...
			if(processingMode != "Handled")
				this.CustomActionCore(evt, "RB", rowId, "0", processOnServer);
		}		
	}
...
function GetGridCollection(){
	if(__ASPxGridCollection == null){
		__ASPxGridCollection = new ASPxClientCollection();
	}
