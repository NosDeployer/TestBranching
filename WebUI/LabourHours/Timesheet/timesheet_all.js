...
function tkrpbLeftMenuItemClick(sender, eventArgs) {
    ...
    switch (item) {
        ...
        case "nbReportsPrintSummaryCostingSheetByMonth":
            window.open("./../projecttime/print_summary_costingsheet_by_month.aspx?source_page=timesheet.aspx", "_blank", "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=yes, copyhistory=no, width=550, height=420");
            eventArgs.set_cancel(true);
            break;
    }
}