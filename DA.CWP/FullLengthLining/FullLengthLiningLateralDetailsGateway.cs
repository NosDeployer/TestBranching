...
namespace LiquiForce.LFSLive.DA.CWP.FullLengthLining
{
    ...
    /// </summary>
    public class FullLengthLiningLateralDetailsGateway : DataTableGateway
    {
        // ////////////////////////////////////////////////////////////////////////
        // CONSTRUCTORS
        //
        ...
        /// Constructor
        /// </summary>
        public FullLengthLiningLateralDetailsGateway()
            : base("LateralDetails")
        {
        }
        ...
        public FullLengthLiningLateralDetailsGateway(DataSet data)
        ...
        protected override void InitAdapter()
        {
            ...
            tableMapping.ColumnMappings.Add("ClientInspectionNo", "ClientInpsectionNo");
        ...
        public DataSet LoadAllByWorkId(int workId, int companyId)
        {
            string command = string.Format(" SELECT LWFM1L.Lateral, LWFM1L.VideoDistance, LWFM1L.ClockPosition, LWFM1L.DistanceToCentre, " +
                    "  LWFM1L.TimeOpened, LWFM1L.ReverseSetup, LWFM1L.Reinstate, LWFM1L.Comments, AASL.LateralID, AASL.Size_, AASL.DistanceFromUSMH,  " +
                    ...
                    "   LWFM1L.COMPANY_ID, 1 AS InProject,  1 AS InProjectDatabase, 1 AS InDatabase,  " +
                    "   AASL.State AS Live,  0 AS ToProcess, "+
                    "   LASLC.ClientLateralID, AASL.ConnectionType, AASL.Address AS Mn, LWFM1L.ClientInspectionNo, LWFM1L.V1Inspection, " +
                    "   LWFM1L.RequiresRoboticPrep, LWFM1L.RequiresRoboticPrepDate, LWFM1L.HoldClientIssue, LWFM1L.HoldLFSIssue, LWFM1L.LineLateral, null as Flange, LWFM1L.DyeTestReq, LWFM1L.DyeTestComplete, LWFM1L.ContractYear " +
        ...
        public DataSet LoadInWorkForEdit(int workId, int sectionId, int companyId)
        {
            string command = string.Format("SELECT AASL.AssetID AS Lateral, LWFM1L.VideoDistance, LWFM1L.ClockPosition, LWFM1L.DistanceToCentre, "+
                   "    LWFM1L.TimeOpened, LWFM1L.ReverseSetup, LWFM1L.Reinstate, LWFM1L.Comments, 'FL-' + AASL.LateralID AS LateralID, " +
                   ...
                   "     AASL.COMPANY_ID, CAST (1 as bit) AS InProject, CAST (1 as bit) AS InProjectDatabase,  CAST (1 as bit) AS InDatabase, AASL.State AS Live, "+
                   "     LASLC.ClientLateralID, AASL.ConnectionType, AASL.Address AS Mn, LWFM1L.ClientInspectionNo, LWFM1L.V1Inspection, " +
                   ...
                   "    LFS_WORK LW2 ON AASL.AssetID = LW2.AssetID AND LW2.Deleted = 0 AND LW2.ProjectID = P.ProjectID LEFT JOIN" +
                   "    LFS_WORK_JUNCTIONLINING_LATERAL LWJLL ON LW2.WorkID = LWJLL.WorkID " +
                   " "+
                   "  WHERE (AASL.Section_ = {1}) AND (LWFM1L.WorkID ={0}) AND (LWFM1L.COMPANY_ID = {2}) AND (LWFM1L.Deleted = 0)  ", workId, sectionId, companyId);
        ...
        public DataSet LoadNotInWorkForEdit(int workId, int sectionId, int companyId, int projectId)
        {
            string command = string.Format(
                   "  SELECT distinct AASL.AssetID AS Lateral, '', '', '', " +
                   "     '', '', '', '', 'JL-' + AASL.LateralID AS LateralID, AASL.Size_, AASL.DistanceFromUSMH,  " +
                   "     AASL.DistanceFromDSMH,  " +
                   "         (SELECT     TOP 1 MaterialType " +
                   "           FROM          AM_ASSET_SEWER_MATERIAL AASM " +
                   "           WHERE      (AASM.AssetID = AASL.AssetID) AND " +
                   "                      (AASM.Deleted = 0) AND (AASM.COMPANY_ID = {2}) " +
                   "           ORDER BY AASM.Date_ DESC) AS MaterialType, AASL.Deleted, " +
                   "     AASL.COMPANY_ID, CAST (0 as bit) AS InProject, CAST (0 as bit) AS InProjectDatabase,  CAST (1 as bit) AS InDatabase, AASL.State AS Live, "+
                   "     LASLC.ClientLateralID, AASL.ConnectionType, AASL.Address AS Mn, '' , '', " +
                   "     CAST (0 as bit) AS RequiresRoboticPrep, '', CAST (0 as bit) AS HoldClientIssue, CAST (0 as bit) AS HoldLFSIssue, CAST (0 as bit) AS LineLateral, null as Flange, CAST (0 as bit) DyeTestReq, null as DyeTestComplete, null as ContractYear" + 
                   "  FROM AM_ASSET_SEWER_LATERAL AASL INNER JOIN " +
                   ...
                   "       LFS_WORK LW2 ON AASL.AssetID = LW2.AssetID AND LW2.Deleted = 0 AND LW2.ProjectID = P.ProjectID LEFT JOIN" +
                   "       LFS_WORK_JUNCTIONLINING_LATERAL LWJLL ON LW2.WorkID = LWJLL.WorkID " +
                   "  WHERE (AASL.Section_ = {1}) AND (AASL.COMPANY_ID = {2}) AND (AASL.Deleted = 0) AND (LW.Deleted = 0)  AND (AASL.AssetID NOT IN " +
                   "  ( " +
                   "      SELECT AASL.AssetID " +
        ...
        public DataRow GetRow(int lateral)
        {
            ...
            {
                throw new Exception("Unavailable row in LiquiForce.LFSLive.DA.CWP.FullLengthLining.FullLengthLiningLateralDetailsGateway.GetRow");
            }
        }
