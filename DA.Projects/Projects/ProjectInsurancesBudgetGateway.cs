...
{
    /// <summary>
    /// ProjectInsurancesBudgetGateway
    /// </summary>
    public class ProjectInsurancesBudgetGateway : DataTableGateway
    {
        // ////////////////////////////////////////////////////////////////////////
        // CONSTRUCTORS
        //
        ...
        /// Constructor
        /// </summary>
        public ProjectInsurancesBudgetGateway()
        ...
        public ProjectInsurancesBudgetGateway(DataSet data)
...
SELECT ProjectID, InsuranceCompanyID, RefID, Budget, Deleted, COMPANY_ID FROM LFS_PROJECT_INSURANCES_BUDGET WHERE (ProjectID = @ProjectID) AND (RefID = @RefID) AND (InsuranceCompanyID = @InsuranceCompanyID)";
...
SELECT ProjectID, InsuranceCompanyID, RefID, Budget, Deleted, COMPANY_ID FROM LFS_PROJECT_INSURANCES_BUDGET WHERE (ProjectID = @ProjectID) AND (RefID = @RefID) AND (InsuranceCompanyID = @InsuranceCompanyID)";
        ...
        {
            FillDataWithStoredProcedure("LFS_PROJECTS_PROJECTINSURANCESBUDGETGATEWAY_LOADALLBYPROJECTID", new SqlParameter("@projectId", projectId));
            return Data;
        }
        ...
        public int Insert(int projectId, int insuranceCompanyId, int refId, decimal budget, bool deleted, int companyId)
        {
            ...
            string command = "INSERT INTO [dbo].[LFS_PROJECT_INSURANCES_BUDGET] ([ProjectID], [InsuranceCompanyID], [RefID], [Budget], [Deleted], [COMPANY_ID]) VALUES (@ProjectID, @InsuranceCompanyID, @RefID, @Budget, @Deleted, @COMPANY_ID)";

            int rowsAffected = (int)ExecuteNonQuery(command, projectIdParameter, insuranceCompanyIdParameter, refIdParameter, budgetParameter, deletedParameter, companyIdParameter);
        ...
        /// <returns>rowsAffected</returns>
        public int Update(int originalProjectId, int originalInsuranceCompanyId, int originalRefId, decimal originalBudget, bool originalDeleted, int originalCompanyId, int newProjectId, int newInsuranceCompanyId, int newRefId, decimal newBudget, bool newDeleted, int newCompanyId)
        {
            ...
            string command = "UPDATE [dbo].[LFS_PROJECT_INSURANCES_BUDGET] SET [Budget] = @Budget " +
                " WHERE (([ProjectID] = @Original_ProjectID) AND ([RefID] = @Original_RefID) AND ([InsuranceCompanyID] = @Original_InsuranceCompanyID))";

            int rowsAffected = (int)ExecuteNonQuery(command, originalProjectIdParameter, originalInsuranceCompanyIdParameter, originalRefIdParameter, originalBudgetParameter, originalDeletedParameter, originalCompanyIdParameter, newProjectIdParameter, newInsuranceCompanyIdParameter, newRefIdParameter, newBudgetParameter, newDeletedParameter, newCompanyIdParameter);
            if (rowsAffected == 0)
            {
        ...
        /// <param name="originalCompanyId">originalCompanyId</param>
        /// <returns>int</returns>
        public int Delete(int originalProjectId, int originalInsuranceCompanyId, int originalRefId, int originalCompanyId)
        {
            SqlParameter originalProjectIdParameter = new SqlParameter("@Original_ProjectID", originalProjectId);
            SqlParameter originalInsuranceCompanyIdParameter = new SqlParameter("@Original_InsuranceCompanyID", originalInsuranceCompanyId);
            SqlParameter originalRefIdParameter = new SqlParameter("@Original_RefID", originalRefId);
            ...
            string command = "UPDATE [dbo].[LFS_PROJECT_INSURANCES_BUDGET] SET  [Deleted] = @Deleted  " +
                             " WHERE (([ProjectID] = @Original_ProjectID) AND ([RefID] = @Original_RefID) AND ([InsuranceCompanyID] = @Original_InsuranceCompanyID) AND " +
                             "  ([COMPANY_ID] = @Original_COMPANY_ID))";

            int rowsAffected = (int)ExecuteNonQuery(command, originalProjectIdParameter, originalInsuranceCompanyIdParameter, originalRefIdParameter, originalCompanyIdParameter, deletedParameter);
