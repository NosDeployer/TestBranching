...
{
    /// <summary>
    /// ProjectCostingSheetSubcontractorsGateway
    /// </summary>
    public class ProjectCostingSheetSubcontractorsGateway : DataTableGateway
    {
        ////////////////////////////////////////////////////////////////////////
        // CONSTRUCTORS
        //
        ...
        /// Constructor
        /// </summary>
        public ProjectCostingSheetSubcontractorsGateway()
            : base("LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR")
        {
        }
        ...
        public ProjectCostingSheetSubcontractorsGateway(DataSet data)
            : base(data, "LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR")
        {
        }
        ...
        protected override void InitAdapter()
        {
            ...
            tableMapping.DataSetTable = "LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR";
            ...
            this._adapter.InsertCommand.CommandText = @"INSERT INTO [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] ([CostingSheetID], [Work_], [SubcontractorID], [RefID], [UnitOfMeasurement], [Quantity], [CostCad], [TotalCostCad], [CostUsd], [TotalCostUsd], [Deleted], [COMPANY_ID], [StartDate], [EndDate], [Function_]) VALUES (@CostingSheetID, @Work_, @SubcontractorID, @RefID, @UnitOfMeasurement, @Quantity, @CostCad, @TotalCostCad, @CostUsd, @TotalCostUsd, @Deleted, @COMPANY_ID, @StartDate, @EndDate, @Function_);
SELECT CostingSheetID, Work_, SubcontractorID, RefID, UnitOfMeasurement, Quantity, CostCad, TotalCostCad, CostUsd, TotalCostUsd, Deleted, COMPANY_ID, StartDate, EndDate, Function_ FROM LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR WHERE (CostingSheetID = @CostingSheetID) AND (RefID = @RefID) AND (SubcontractorID = @SubcontractorID) AND (Work_ = @Work_)";
            ...
            this._adapter.UpdateCommand.CommandText = "UPDATE [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] SET [CostingSheetID] = @CostingShe" +
                "etID, [Work_] = @Work_, [SubcontractorID] = @SubcontractorID, [RefID] = @RefID, [UnitOfMeasurement" +
                ...
                "ingSheetID) AND ([Work_] = @Original_Work_) AND ([SubcontractorID] = @Original_SubcontractorID) AN" +
                ...
                "SELECT CostingSheetID, Work_, SubcontractorID, RefID, UnitOfMeasurement, Quantity, CostCa" +
                "d, TotalCostCad, CostUsd, TotalCostUsd, Deleted, COMPANY_ID, StartDate, EndDate," +
                " Function_ FROM LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR WHERE (CostingSheetID = @Costing" +
                "SheetID) AND (RefID = @RefID) AND (SubcontractorID = @SubcontractorID) AND (Work_ = @Work_)";
        ...
        public void Insert(int costingSheetId, int subcontractorID, int refId, string unitOfMeasurement, double quantity, decimal costCad, decimal totalCostCad, decimal costUsd, decimal totalCostUsd, bool deleted, int companyId, DateTime startDate, DateTime endDate, string comment)
        {
            ...
            string command = "INSERT INTO [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] ([CostingSheetID], [SubcontractorID], [RefID], [UnitOfMeasurement], [Quantity], [CostCad], "+
                             " [TotalCostCad], [CostUsd], [TotalCostUsd], [Deleted], [COMPANY_ID], [StartDate], [EndDate], [Comment]) VALUES (@CostingSheetID, @SubcontractorID, @RefID, @UnitOfMeasurement, "+
                             " @Quantity, @CostCad, @TotalCostCad, @CostUsd, @TotalCostUsd, @Deleted, @COMPANY_ID, @StartDate, @EndDate, @Comment)";
            
            ExecuteScalar(command, costingSheetIdParameter, subcontractorIDParameter, refIdParameter, unitOfMeasurementParameter, quantityParameter, costCadParameter, totalCostCadParameter, costUsdParameter, totalCostUsdParameter, deletedParameter, companyIdParameter, startDateParameter, endDateParameter, commentParameter);
        }
        ...
        public void Update(int costingSheetId, int subcontractorID, int refId, string originalUnitOfMeasurement, double originalQuantity, decimal originalCostCad, decimal originalTotalCostCad, decimal originalCostUsd, decimal originalTotalCostUsd, bool originalDeleted, int originalCompanyId, DateTime originalStartDate, DateTime originalEndDate, string newUnitOfMeasurement, double newQuantity, decimal newCostCad, decimal newTotalCostCad, decimal newCostUsd, decimal newTotalCostUsd, bool newDeleted, int newCompanyId, DateTime newStartDate, DateTime newEndDate)
        {
            ...
            SqlParameter newUnitIdParameter = new SqlParameter("SubcontractorID", subcontractorID);
            ...
            string command = "UPDATE [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] SET [CostingSheetID] = @CostingSheetID, [SubcontractorID] = @SubcontractorID, [RefID] = @RefID, [UnitOfMeasurement] = @UnitOfMeasurement, "+
                ...
                " WHERE (" +
                " ([CostingSheetID] = @Original_CostingSheetID) AND ([SubcontractorID] = @Original_SubcontractorID) AND ([RefID] = @Original_RefID) AND ([COMPANY_ID] = @Original_COMPANY_ID) " +
                " )";

            int rowsAffected = (int)ExecuteNonQuery(command, originalCostingSheetIdParameter, originalUnitIdParameter, originalRefIdParameter, originalUnitOfMeasurementParameter, originalCostCadParameter, originalTotalCostCadParameter, originalCostUsdParameter, originalTotalCostUsdParameter, originalQuantityParameter, originalDeletedParameter, originalCompanyIdParameter, originalStartDateParameter, originalEndDateParameter, newCostingSheetIdParameter, newUnitIdParameter, newRefIdParameter, newUnitOfMeasurementParameter, newCostCadParameter, newTotalCostCadParameter, newCostUsdParameter, newTotalCostUsdParameter, newQuantityParameter, newDeletedParameter, newCompanyIdParameter, newStartDateParameter, newEndDateParameter);
            if (rowsAffected == 0)
            {
        ...
        public int DeleteAll(int originalcostingSheetId, int originalCompanyId)
        {
            ...
            string command = "UPDATE [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] SET  [Deleted] = @Deleted  " +
        ...
        /// <param name="originalCompanyId">originalCompanyId</param>
        /// <returns>int</returns>
        public int Delete(int originalCostingSheetId, int originalUnitId, int originalRefId, int originalCompanyId)
        {
            SqlParameter originalCostingSheetIdParameter = new SqlParameter("Original_CostingSheetID", originalCostingSheetId);
            SqlParameter originalUnitIdParameter = new SqlParameter("Original_SubcontractorID", originalUnitId);
            SqlParameter originalRefIdParameter = new SqlParameter("Original_RefID", originalRefId);
            SqlParameter originalCompanyIdParameter = new SqlParameter("Original_COMPANY_ID", originalCompanyId);
            SqlParameter deletedParameter = new SqlParameter("Deleted", 1);

            string command = "UPDATE [dbo].[LFS_PROJECT_COSTING_SHEET_SUBCONTRACTOR] SET  [Deleted] = @Deleted " +
                             " WHERE (([CostingSheetID] = @Original_CostingSheetID) AND ([SubcontractorID] = @Original_SubcontractorID) AND ([RefID] = @Original_RefID) AND " +
                             "  ([COMPANY_ID] = @Original_COMPANY_ID))";

            int rowsAffected = (int)ExecuteNonQuery(command, originalCostingSheetIdParameter, originalUnitIdParameter, originalRefIdParameter, originalCompanyIdParameter, deletedParameter);
